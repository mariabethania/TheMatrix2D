...
public class TheMatrix2D extends PApplet {
...
//}
//  timer.countDown();
//  if (timer.getTime() <=10) { ps.addSource(); ps.runSource(); }
//  if (timer.getTime() <= 6) { ps.addSource1(); ps.runSource1();}
//  if (timer.getTime() <= 4) { ps.addSource2(); ps.runSource2(); }
//  if (timer.getTime() <= 3) { ps.addSource3(); ps.runSource3();   }
//stroke(255); sphere(2+fft.getBand(12)); 
...
class Particle {
  ...
  public void display() {
    ...
    text(PApplet.parseChar(PApplet.parseInt( random(33,126))),location.x,location.y - (letras.yspeed * random(1,10)),location.z);
    } else {
      //ch = temp;
...
public void countDown() {
 Time -= 1/frameRate; 
  ...
  public void settings() {  size(1024, 640,P2D); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "TheMatrix2D" };
    if (passedArgs != null) {
